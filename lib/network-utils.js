...
function getIpAddressForNetworkInterface(networkInterfaceName) {
  ...
  if (networkInterface) {
    ...
    var validInterface = _.find(networkInterface, function(currInterface) {
      // skip any localhost or non-IPv4 addresses
      if ('IPv4' !== currInterface.family || currInterface.internal !== false) {
        return false;
      } else {
        return true;
      }
    ...
    if (validInterface) {
      ...
      ipAddress = validInterface.address;
    }
  }
